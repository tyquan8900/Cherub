...
(() => {
  ...
  function openVisual(domain, card) {
    ...
    box.innerHTML = `<h2>${v[0]}</h2><p class="tiny">Original Cherub study visual based on the mapped relationship data.</p><div style="display:flex;flex-wrap:wrap;gap:7px;align-items:center;margin:12px 0">${v[1].split('→').map((x,i)=>`<span style="display:flex;align-items:center;gap:7px"><b style="padding:8px 10px;border-radius:9px;background:${i===0?'#e7f0ff':i===v[1].split('→').length-1?'#e8f8ee':'#f4efff'};font-size:12px">${x.trim()}</b>${i<v[1].split('→').length-1?'<strong style="color:#2f72e6">→</strong>':''}</span>`).join('')}</div><table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr><th style="padding:8px;border:1px solid #dce6f3;background:#edf5ff;text-align:left">Visual reference</th><th style="padding:8px;border:1px solid #dce6f3;background:#edf5ff;text-align:left">What it shows</th></tr></thead><tbody>${v[2].map(r=>`<tr><td style="padding:8px;border:1px solid #dce6f3">${r[0]}</td><td style="padding:8px;border:1px solid #dce6f3">${r[1]}</td></tr>`).join('')}</tbody></table>`;
    card.append(box); box.scrollIntoView({behavior:'smooth',block:'nearest'});
  }
  window.CherubVisualCatalog = { open(domain) { window.show('resources'); setTimeout(() => { const card=document.querySelector('#resources .placeholder'); if (card) openVisual(domain, card); }, 80); } };
