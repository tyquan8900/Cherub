...
(() => {
  ...
  function openVisual(domain, card, selected, focus) {
    ...
    const focusedFlow = focus?.concept ? [focus.concept, ...(focus.related||[]).slice(0,3)].filter(Boolean).join(' → ') : '';
    ...
    const title=focus?.concept ? `${focus.concept}: connected decision map` : (selected||v[0]);
