...
(() => {
  ...
  function show(focus) {
    ...
    host.insertAdjacentHTML('afterbegin', `<div class="notice cherub-builder"><h2 style="margin:0 0 6px">Build a focused practice set</h2><p><b>${label}</b> • ${domain[focus.domain] || focus.domain}</p><label>Questions: <b id="setCount">10</b></label><input id="setCountRange" type="range" min="5" max="150" step="5" value="10" style="width:100%"><label>Challenge level: <b id="setLevel">10</b> / 20</label><input id="setLevelRange" type="range" min="1" max="20" value="10" style="width:100%"><p class="tiny">Higher levels use more difficult scenario variants. This set is resumable and keeps answer, confidence, reasoning, and distractor analysis.</p><button class="enginebtn" id="startFocusedSet" ${ready ? '' : 'disabled'}>${ready ? 'Start focused set' : 'Complete 20Q diagnostic to unlock'}</button></div>`);
