...
(() => {
  ...
  window.addEventListener('cherub:view', e => {
    ...
    setTimeout(() => {
      ...
      card.prepend(block);
      const draw = () => { const start = new Date(`${block.querySelector('#cherubStartDate').value}T12:00:00`); const rows = [[0, 'Week 1', '20-question cold diagnostic'], [7, 'Week 2', 'Adaptive focused checkpoints'], [14, 'Week 3', 'Fresh 150-question full exam'], [21, 'Week 4', 'Fresh 150-question full exam'], [28, 'Week 5', 'Fresh 150-question full exam'], [35, 'Week 6', 'Fresh 150-question full exam']]; block.querySelector('#cherubTimeline').innerHTML = rows.map(([days, week, detail]) => { const d = new Date(start); d.setDate(d.getDate() + days); return `<div class="topicline"><span><b>${week}</b> — ${detail}</span><span class="epill">${fmt(d)}</span></div>`; }).join(''); };
