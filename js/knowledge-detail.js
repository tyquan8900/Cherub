...
(() => {
  ...
  window.addEventListener("cherub:knowledge", (e) => {
    ...
    const officialSource = x.domain === 'D4' ? { label:'NIST SP 800-34 Rev. 1', url:'https://csrc.nist.gov/pubs/sp/800/34/r1/final' } : x.domain === 'D2' ? { label:'NIST SP 800-30 Rev. 1', url:'https://csrc.nist.gov/pubs/sp/800/30/r1/final' } : { label:'NIST Cybersecurity Framework 2.0', url:'https://www.nist.gov/cyberframework' };
    card.insertAdjacentHTML('beforeend', `<section aria-label="Visual references" style="margin-top:12px;padding:11px;border-radius:10px;background:#fffdf5;border:1px solid #f0dfad"><b style="font-size:13px">Visual aids and source reference</b><p class="tiny" style="margin:6px 0">Cherub map: original learning aid. Official public reference: <a href="${officialSource.url}" target="_blank" rel="noopener noreferrer">${officialSource.label} ↗</a></p><div style="display:grid;gap:5px;margin-top:8px">${visualRefs.map(ref => `<button data-visual-ref class="atlas-link" style="width:100%">${ref}<b>Open Cherub visual</b></button>`).join('')}</div></section>`);
