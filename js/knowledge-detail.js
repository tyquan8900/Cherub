...
(() => {
  ...
  window.addEventListener("cherub:knowledge", (e) => {
    ...
    card.insertAdjacentHTML('beforeend', `<section aria-label="Visual references" style="margin-top:12px;padding:11px;border-radius:10px;background:#fffdf5;border:1px solid #f0dfad"><b style="font-size:13px">Visual aids and source reference</b><p class="tiny" style="margin:6px 0">Each visual opens a diagram centered on <b>${t}</b>. Domain blueprint: ${officialSources.map(s=>`<a href="${s.url}" target="_blank" rel="noopener noreferrer">${s.label} ↗</a>`).join(' &nbsp;•&nbsp; ')}</p><div style="display:grid;gap:7px;margin-top:8px">${visualRefs.map(ref => `<button data-visual-ref data-ref="${ref}" style="width:100%;display:flex;justify-content:space-between;align-items:center;gap:12px;padding:9px 11px;border:1px solid #b7cbe6;border-radius:8px;background:#fff;color:#194b86;font-weight:700;cursor:pointer;text-align:left"><span>${ref}</span><span style="white-space:nowrap;color:#2f72e6">Open diagram →</span></button>`).join('')}</div></section>`);
    ...
    card.querySelectorAll('[data-visual-ref]').forEach(button => button.onclick = () => window.CherubVisualCatalog?.open(x.domain || c?.domain || 'D1', button.dataset.ref, visualFocus));
