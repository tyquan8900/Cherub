...
(() => {
  ...
  window.addEventListener("cherub:knowledge", (e) => {
    ...
    card.insertAdjacentHTML('beforeend', `<section aria-label="Visual references" style="margin-top:12px;padding:11px;border-radius:10px;background:#fffdf5;border:1px solid #f0dfad"><b style="font-size:13px">Visual aids and source reference</b><p class="tiny" style="margin:6px 0">Cherub map: original learning aid. Domain blueprint: ${officialSources.map(s=>`<a href="${s.url}" target="_blank" rel="noopener noreferrer">${s.label} ↗</a>`).join(' &nbsp;•&nbsp; ')}</p><div style="display:grid;gap:5px;margin-top:8px">${visualRefs.map(ref => `<button data-visual-ref class="atlas-link" style="width:100%">${ref}<b>Open Cherub visual</b></button>`).join('')}</div></section>`);
